...
@Service
@Slf4j
@ConfigurationProperties(prefix = "customer.baidu-image")
public class BaiduAuthService {
    ...
    public String getAuth() {
        ...
        String redisKey = RedisKeyRules.baiduImageToken();
        if(redisCommonService.exists(redisKey)) return (String) redisStringService.get(redisKey);

        String token = getAuth(clientId, clientSecret);
        redisStringService.add(redisKey, token, 30 * 24 * 3600L);
    ...
    public String getAuth(String ak, String sk) {
        // 获取token地址
        String authHost = host + "/oauth/2.0/token?";
