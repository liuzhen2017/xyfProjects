...
public class SyncDataService {
      ...
      private final JDGoodsDetailParam globalDetailParam = new JDGoodsDetailParam();
      ...
      public void syncGoodsInfo() {
            ...
            if (CollectionUtils.isNotEmpty(skuIdList)) {
                  for (String skuId : skuIdList) {
                        ...
                        try {
                              //先根据 skuId查询 proId，再根据 proId 查出 product
                              ResponseInfo<Product> resProduct = productManageFeign.getProductBySkuNo(skuId);
      ...
      public void syncGoodsAttr() {
            ...
            for (String skuId : skuIdList) {
                  ...
                  //根据proNo从本地库查询 proId
                  try {
                        ResponseInfo<Long> resProId = productManageFeign.getProIdBySkuNo(skuId);
      ...
      public List<JDGoodsDto> getGoosList() {
            ...
            try {
                  ...
                  //ResponseInfo<JDGoodsListDto> response = jdGateWayService.getGoodsList(goodsListParam);
                  //JDGoodsListDto goodsListDto = response.getData();
                  JDGoodsListDto goodsListDto = jdGateWayService.getGoodsList(goodsListParam);
                  if (goodsListDto != null) {
                        JDGoodsListDto pageListDto;
                        ...
                        while (goodsListDto.getTotal() > (goodsListParam.getPage() + 1) * goodsListParam.getPagesize()) {
                              ...
                              //ResponseInfo<JDGoodsListDto> res = jdGateWayService.getGoodsList(goodsListParam);
                              //pageListDto = res.getData();
                              pageListDto = jdGateWayService.getGoodsList(goodsListParam);
                              if (pageListDto != null) {
                                    List<JDGoodsDto> pageList = pageListDto.getData();
                                    if (CollectionUtils.isNotEmpty(pageList)) {
                                          jdGoodsDtos.addAll(pageList);
                                    }
                              }
                        }
                  }
      ...
      public void wrapperGoodsAndIntoDB(JDGoodsDto goodsDto) {
            if (goodsDto != null) {
                  ...
                  //先查询，不存在再进行后续操作
                  try {
                        ResponseInfo<Boolean> responsePro = productManageFeign.checkProByProNo(proNo);
                        //已存在不做操作，不存在则insert
                        if (!responsePro.getData()) {
                              ...
                              try {
                                    ResponseInfo<Product> resPro = productManageFeign.queryProductByProNo(proNo);
                                    Product resProduct = resPro.getData();
                                    if (resProduct != null) {
                                          Long proId = resProduct.getProId();
                                          ...
                                          if (category != null) {
                                                ...
                                                Long channelId = Long.valueOf(category.substring(temp + 1));
                                                //根据 channelId查询 channelName
                                                ResponseInfo<String> response = productManageFeign.getChannelNameByChannelId(channelId);
                                                if (response.getCode() == "0000") {
                                                      channelName = response.getData();
                                                }
                                                ProChannelRelation pcr = ProChannelRelation.builder()
                                                          .proId(proId).proName(resProduct.getProName()).channelId(channelId).channelName(channelName).build();
...
/*                  Product product = Product.builder().proNo(proNo).proName(goodsDto.getName()).status(99).source(1).build();
      ...
      public void getGoodsDetailInfo(String skuId, Product product, ProDetails proDetails) {
            ...
            globalDetailParam.setSku(skuId);
            JDGoodsDetailDto goodsDetailDto = jdGateWayService.getGoodsDetail(globalDetailParam);
...
//————————————————入库———————————————————————————————————————————————
       ...
       * ProChannelRelation 入库
       */
      public void proCRIntoDb(ProChannelRelation pcr, Long proId, Long channelId) {
            try {
