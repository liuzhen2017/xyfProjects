...
public class SysUserController extends BaseController {
    ...
    public AjaxResult resetPayPwdSave(SysUser user) {
      ...
      @RequiresPermissions("system:user:savePayPwd")
      @Log(title = "保存支付密码", businessType = BusinessType.UPDATE)
      @GetMapping("/savePayPwd/{userId}")
      public String savePayPwd(@PathVariable("userId") Long userId, ModelMap mmap) {
            mmap.put("user", userService.selectUserById(userId));
            return prefix + "/savePayPwd";
      }
      ...
      @ResponseBody
      public AjaxResult savePayPwd(String oldPayPassword, String payPassword)
      {
            SysUser user = ShiroUtils.getSysUser();
            String dbPayPwd = userService.getPayPwdByUserId(user.getUserId());

            if(dbPayPwd.equals(Md5Utils.hash(oldPayPassword))){
                  user.setPayPassword(Md5Utils.hash(payPassword));
                  if (userService.resetUserPayPwd(user) > 0)
                  {
                        ShiroUtils.setSysUser(userService.selectUserById(user.getUserId()));
                        return success();
    ...
    public AjaxResult checkPayPwd(String payPwd) {
      ...
      @ResponseBody
      public boolean verifyPayPwd(String password) {

            AjaxResult result=userService.checkPayPwd(ShiroUtils.getUserId(),password);
          logger.info("reslut={}",result.toString());
          //result.getType();
