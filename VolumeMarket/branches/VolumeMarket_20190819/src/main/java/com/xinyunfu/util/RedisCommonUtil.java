...
public class RedisCommonUtil {
	...
	private static final String dis_key="VolumeMarket_";
    @Autowired
    ...
    public boolean setCache(String key, Object value) {
        try {
            redisTemplate.opsForValue().set(dis_key+key, value);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public boolean setCache(String key, Object value, long time) {
        try {
            if (time > 0) {
                redisTemplate.opsForValue().set(dis_key+key, value, time, TimeUnit.SECONDS);
            } else {
                setCache(dis_key+key, value);
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Object getHashCache(String key, String item) {
        return redisTemplate.opsForHash().get(dis_key+key, item);
    }
    ...
    public Map<Object, Object> getHashCache(String key) {
        return redisTemplate.opsForHash().entries(dis_key+key);
    }
    ...
    public boolean setHashCache(String key, Map<String, Object> map) {
        try {
            redisTemplate.opsForHash().putAll(dis_key+key, map);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public boolean setHashCache(String key, Map<String, Object> map, long time) {
        try {
            redisTemplate.opsForHash().putAll(dis_key+key, map);
            if (time > 0) {
                setExpireTime(dis_key+key, time);
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean setHashCache(String key, String item, Object value) {
        try {
            redisTemplate.opsForHash().put(dis_key+key, item, value);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public boolean setHashCache(String key, String hashKey, Object value, long time) {
        try {
            redisTemplate.opsForHash().put(dis_key+key, hashKey, value);
            if (time > 0) {
                setExpireTime(dis_key+key, time);
            }
            return true;
