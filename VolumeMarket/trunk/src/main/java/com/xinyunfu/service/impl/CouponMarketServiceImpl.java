...
public class CouponMarketServiceImpl extends ServiceImpl<CouponMarketMapper, CouponMarket>
	...
	public ResponseInfo<IPage<Map<String, Object>>> queryByGroupList(Integer page, Integer pageSize,
		...
		for(int i=0;i<records.size(); i++) {
		    temp =records.get(i);
		    if (SysConstant.coupon_status_use_ed01.equals(couponStatus)) {
		    	useDate =(Date)temp.getOrDefault("useDate", new Date());
		    	temp.put("useDateTemp", DateFormatUtils.format(useDate,"yyyy-MM-dd HH:mm:ss"));
//		    	temp.put("signDateTemp", DateFormatUtils.format(useDate,"yyyy-MM-dd HH:mm:ss"));		    				    
		    }else {
		    	signDate =(Date)temp.getOrDefault("signDate", new Date());
		    	temp.put("useDateTemp", DateFormatUtils.format(signDate,"yyyy-MM-dd HH:mm:ss"));
//		    	temp.put("signDateTemp", DateFormatUtils.format(signDate,"yyyy-MM-dd HH:mm:ss"));
		    }
...
//		couponMarketMapper.updateByByOrder(SysConstant.coupon_status_use_ed01, orderNo);
		...
		Future<ResponseInfo<String>> result = executor.submit(() -> {
			try {
				if(redisLock.lock()) {
					...
					// 1.根据用户类型,查询订单
					if (level == 1) {
						lastMarketByUser = couponMarketMapper.queryFirstMarketByUser(recvUserNo,1);
					} else {
					...
					if (Integer.parseInt(orderNo.split("_")[1])%6 == 0 ) {
						...
						map.put("sendType", "02");
						map.put("sendContent", "恭喜您：您的券已全部转让成功！获得了再次购买超级礼包的资格，及时购买获得更多收益!");
						rabbitTemplate.convertAndSend("InfoCenter_sendSMS", JSONObject.toJSONString(map));
						
						//修改下一笔的数据
						if (level == 1) {
							log.info("===用户等级为 大使,执行修改用户剩下的券转让时间为=={}==",new Date());
							List<CouponMarket> nextMarketByUser = couponMarketMapper.queryFirstMarketByUser(recvUserNo,6);
							nextMarketByUser.forEach(s->{s.setSignDate(new Date());});
							this.updateBatchById(nextMarketByUser);
	...
	public ResponseInfo<String> backTransferAccounts(String orderId, Integer status) {
		log.info("===处理支付中心回调==orderId={},status={}",orderId,status);
		...
		try {
		   if(redisLock.lock()) {
				...
				if (!SysConstant.coupon_status_to_be_recorded.equals(coupon.getCouponStatus())) {
		...
		executor.submit(() -> {
			...
			try {
				if(redisLock.lock()) {
					...
					// 1.根据用户类型,查询订单
					if (level == 1) {
						lastMarketByUser = couponMarketMapper.queryFirstMarketByUser(userNo,1);
					} else {
					...
					map.put("sendType", "02");
					//您的粉丝xxx又购买了礼包，因您没有及时购买礼包，已经错过几次券转让的机会
					map.put("sendContent", "您的粉丝"+dto.getNickName()+"又购买了礼包,因您没有及时购买礼包,已经错过几次券转让的机会~");
